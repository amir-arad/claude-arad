...
# Done plugin: second Cowork run (done 0.4.3)
...
- 0.4.3 is released. In Cowork, update the marketplace and check that the picker lists `init-done`, `what-now-done` and `goals-done` under done.
...
| 3 | `/done:what-now-done` | Routes the license DECIDE card under its new id (rule 3). state.md v4 | repeat same day |
| 4 | `/done:init-done` | Asks which folder → B. Interview: name `other`; sync `self-report`; capacity `none`; never: none. `B/.done/` is scaffolded with `sync: self-report` and `capacity: none` | init on the second folder |
...
| 6 | `/done:what-now-done` | Asks which folder → B. No git, sync self-report → asks "What changed since …?" → answer `nothing`. Routes M1.1 (rule 3). capacity none: no dispatch talk | self-report, capacity none |
