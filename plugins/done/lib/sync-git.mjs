...
const HELP = `sync-git.mjs — local git facts for /done:what-now.
...
if (isMain(import.meta.url)) {
  ...
  let logText;
  try {
    logText = execFileSync('git', ['-C', root, 'log', `--since=${since}`, `-n`, String(args.limit ?? 200), `--format=%h${SEP}%as${SEP}%s`],
