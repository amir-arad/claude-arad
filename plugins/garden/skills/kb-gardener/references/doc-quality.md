...
6. **Stated limits** — for a note that makes claims rather than giving directions: does it name
   where its evidence comes from, and what that evidence does not cover? A note whose silence
   reads as coverage is the one a later maintainer cannot correct, because nothing records what
   was never checked.
...
- **Setup completes without help**: A new developer can reach a working local environment using only the generated guide, with no questions asked outside it.
- **Error coverage**: Every error in the debugging guide is a real error string collected from logs, CI, or the issue tracker — not invented.
- **Key file accuracy**: Every file listed in the key file map is either a `git_hotspots` entry from the scan or justified in prose by real coupling/blast-radius you read in the code.
- **Stated limits**: Every claim-making note ends with what its evidence does not establish — sample size, single repository, analogy rather than measurement. Cheap to write while the work is fresh, unrecoverable afterwards.
- **Link validity**: Zero broken internal or external links when validated by a link checker at publish time.
